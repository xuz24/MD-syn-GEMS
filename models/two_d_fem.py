...
class MolecularGCN(nn.Module):
    def __init__(self, input_dim=78, hidden_dims=[512, 128]):
        super().__init__()
...
class Two_D_FEM(nn.Module):
    def __init__(self, ppi_graph_path, drug_targets_path, drug_dbid_mapping_path, drug_smiles_path):
        ...
        if 'smiles' in df_smiles.columns:
            name_cols = [c for c in df_smiles.columns if c.lower() != 'smiles']
            name_col = name_cols[0]
            raw_smiles = df_smiles.set_index(name_col)['smiles'].to_dict()
        else:
            raw_smiles = pd.Series(df_smiles.iloc[:,1].values, index=df_smiles.iloc[:,0].astype(str)).to_dict()
        self.drug_smiles = { _normalize(k): v for k, v in raw_smiles.items() }
        ...
        self.drug_mapping = { _normalize(k): v for k, v in raw_map.items() }
    ...
    def get_ppi_features(self, drug_name): 
        ...
        dbid = self.drug_mapping.get(_normalize(drug_name))
        if dbid is None:
            raise KeyError(f"Drug->dbid mapping not found for: {drug_name}")
        ...
        if len(indices) == 0:
            # return empty tensor (no targets)
            return torch.empty((0, self.ppi['embeddings'].size(1)))
    ...
    def forward(self, drug_1, drug_2):
        ...
        drug_1_smiles = self.drug_smiles.get(_normalize(drug_1))
        drug_2_smiles = self.drug_smiles.get(_normalize(drug_2))
        ...
        if drug_1_ppi.numel() == 0:
            drug_1_ppi = torch.zeros((1, 1, self.gcn.conv2.out_channels), dtype=torch.float)
        else:
            drug_1_ppi = drug_1_ppi.unsqueeze(0).float()

        if drug_2_ppi.numel() == 0:
            drug_2_ppi = torch.zeros((1, 1, self.gcn.conv2.out_channels), dtype=torch.float)
        else:
            drug_2_ppi = drug_2_ppi.unsqueeze(0).float()
